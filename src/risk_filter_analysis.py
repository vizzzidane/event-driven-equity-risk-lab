...
RISK_FILTER_ANALYSIS_PATH = Path("results/risk_filter_analysis.csv")
...
BASE_EVENT_STRENGTH_THRESHOLD = 2.0
BASE_VOLUME_SHOCK_THRESHOLD = 1.2
...
FILTER_CONFIGS = [
    {
        ...
        "exclude_tickers": [],
        "exclude_years": [],
        "max_abs_event_strength": None,
        "max_volume_shock": None,
    },
    {
        "filter_name": "exclude_xom",
        "exclude_tickers": ["XOM"],
        "exclude_years": [],
        "max_abs_event_strength": None,
        "max_volume_shock": None,
    ...
    {
        "filter_name": "exclude_extreme_strength_gt_6",
        "exclude_tickers": [],
        "exclude_years": [],
        "max_abs_event_strength": 6.0,
        "max_volume_shock": None,
    },
    {
        "filter_name": "cap_volume_shock_3",
        "exclude_tickers": [],
        "exclude_years": [],
        "max_abs_event_strength": None,
        "max_volume_shock": 3.0,
    },
    {
        "filter_name": "practical_filter_no_xom_strength6_volume3",
        "exclude_tickers": ["XOM"],
        "exclude_years": [],
        "max_abs_event_strength": 6.0,
        "max_volume_shock": 3.0,
    ...
    {
        "filter_name": "stress_exclusion_no_2022",
        "exclude_tickers": [],
        "exclude_years": [2022],
        "max_abs_event_strength": 6.0,
        "max_volume_shock": 3.0,
...
def add_event_features(prices: pd.DataFrame) -> pd.DataFrame:
    ...
    stock_rows["abs_event_strength"] = stock_rows["event_strength"].abs()
    stock_rows["event_year"] = stock_rows["date"].dt.year

    return stock_rows.sort_values(["ticker", "date"]).reset_index(drop=True)


def detect_base_negative_events(features: pd.DataFrame) -> pd.DataFrame:
    """
    Detect base negative abnormal price-volume events.

    Base rule:
    - event_strength <= -2.0
    - volume_shock >= 1.2
    - avg dollar volume >= 50M
    """
    events = features[
        features["event_strength"].le(-BASE_EVENT_STRENGTH_THRESHOLD)
        & features["volume_shock"].ge(BASE_VOLUME_SHOCK_THRESHOLD)
...
def apply_event_filter(events: pd.DataFrame, config: dict) -> pd.DataFrame:
    """Apply one risk/failure-mode filter config to detected events."""
    out = events.copy()

    exclude_tickers = config.get("exclude_tickers") or []
    exclude_years = config.get("exclude_years") or []
    max_abs_event_strength = config.get("max_abs_event_strength")
    max_volume_shock = config.get("max_volume_shock")

    if exclude_tickers:
        out = out[~out["ticker"].isin(exclude_tickers)].copy()

    if exclude_years:
        out = out[~out["event_year"].isin(exclude_years)].copy()

    if max_abs_event_strength is not None:
        out = out[out["abs_event_strength"].le(max_abs_event_strength)].copy()

    if max_volume_shock is not None:
        out = out[out["volume_shock"].le(max_volume_shock)].copy()
    ...
    for _, event in events.iterrows():
        ...
        trades.append(
            Trade(
                ...
                abs_event_strength=float(event["abs_event_strength"]),
...
def summarize_filter_run(
    ...
    return {
        ...
        "hold_days": HOLD_DAYS,
        "base_n_events": len(base_events),
        "filtered_n_events": len(filtered_events),
        "n_trades": len(trades),
        "excluded_tickers": ",".join(config.get("exclude_tickers") or []),
        "excluded_years": ",".join(str(y) for y in (config.get("exclude_years") or [])),
        "max_abs_event_strength": config.get("max_abs_event_strength"),
        "max_volume_shock": config.get("max_volume_shock"),
...
def summarize_empty_filter_run(
    config: dict,
    base_events: pd.DataFrame,
    filtered_events: pd.DataFrame,
) -> dict[str, float | int | str]:
    """Fallback summary when a filter produces no trades."""
    return {
        "strategy": STRATEGY_NAME,
        "filter_name": config["filter_name"],
        "hold_days": HOLD_DAYS,
...
def run_risk_filter_analysis() -> None:
    """Run practical failure-mode filter analysis."""
    prices = load_prices()
    features = add_event_features(prices)
    base_events = detect_base_negative_events(features)

    rows = []

    for config in FILTER_CONFIGS:
        print(f"Running risk filter: {config['filter_name']}...")

        filtered_events = apply_event_filter(base_events, config)
    ...
    RISK_FILTER_ANALYSIS_PATH.parent.mkdir(parents=True, exist_ok=True)
    summary.to_csv(RISK_FILTER_ANALYSIS_PATH, index=False)

    print()
    print(f"Saved risk filter analysis to: {RISK_FILTER_ANALYSIS_PATH}")

    print()
    print("Risk filter analysis summary")
    print("----------------------------")
    print(
        summary[
            [
