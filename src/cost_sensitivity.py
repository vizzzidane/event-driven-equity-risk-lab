...
COST_SENSITIVITY_PATH = Path("results/cost_sensitivity.csv")
...
COST_BPS_PER_SIDE_GRID = [1.0, 5.0, 10.0, 25.0, 50.0]
...
def summarize_cost_run(
    ...
    """Summarize one cost sensitivity run."""
...
def run_cost_sensitivity() -> None:
    """Run cost sensitivity grid for abnormal-return backtest."""
    prices = load_prices()
    events = load_events()

    rows = []

    for cost_bps in COST_BPS_PER_SIDE_GRID:
        print(f"Running cost sensitivity: {cost_bps} bps per side...")
    ...
    COST_SENSITIVITY_PATH.parent.mkdir(parents=True, exist_ok=True)
    summary.to_csv(COST_SENSITIVITY_PATH, index=False)

    print()
    print(f"Saved cost sensitivity results to: {COST_SENSITIVITY_PATH}")
    ...
    print(
        summary[
            [
                ...
                "transaction_cost_bps_round_trip",
                "n_trades",
                "trade_win_rate",
