...
COOLDOWN_SENSITIVITY_PATH = Path("results/cooldown_sensitivity.csv")
COOLDOWN_TRADE_LEDGER_PATH = Path("results/cooldown_trade_ledger.csv")

STRATEGY_NAME = "negative_event_reversal_30d_abnormal_cooldown"

HOLD_DAYS = 30
MAX_CONCURRENT_POSITIONS = 5
TRANSACTION_COST_BPS_PER_SIDE = 5.0

COOLDOWN_DAYS_GRID = [0, 5, 10, 20, 30]
...
def build_candidate_trades(
    ...
    Cooldown rule:
    after accepting a candidate event for a ticker, ignore subsequent events
    for the same ticker until at least cooldown_days have passed.
    """
    ...
    last_event_date_by_ticker: dict[str, pd.Timestamp] = {}
    trade_id = 1
    ...
    for _, event in events.iterrows():
        ...
        last_event_date = last_event_date_by_ticker.get(ticker)

        if last_event_date is not None:
            days_since_last_event = (event_date - last_event_date).days

            if days_since_last_event < cooldown_days:
                continue
        ...
        last_event_date_by_ticker[ticker] = event_date
        trade_id += 1
...
def run_cooldown_sensitivity() -> None:
    """Run cooldown sensitivity analysis."""
    prices = load_prices()
    events = load_events()
    ...
    for cooldown_days in COOLDOWN_DAYS_GRID:
        print(f"Running cooldown sensitivity: {cooldown_days} calendar days...")
        ...
        if not accepted_trades.empty:
            accepted_trades["cooldown_days"] = cooldown_days
            ledger_frames.append(accepted_trades)

    summary = pd.DataFrame(summary_rows)

    COOLDOWN_SENSITIVITY_PATH.parent.mkdir(parents=True, exist_ok=True)
    summary.to_csv(COOLDOWN_SENSITIVITY_PATH, index=False)

    if ledger_frames:
        ledger = pd.concat(ledger_frames, ignore_index=True)
        ledger.to_csv(COOLDOWN_TRADE_LEDGER_PATH, index=False)

    print()
    print(f"Saved cooldown sensitivity to: {COOLDOWN_SENSITIVITY_PATH}")
    print(f"Saved cooldown trade ledger to: {COOLDOWN_TRADE_LEDGER_PATH}")

    print()
    print("Cooldown sensitivity summary")
    print("----------------------------")
    print(
        summary[
            [
