...
GLOBAL_PACING_SENSITIVITY_PATH = Path("results/global_pacing_sensitivity.csv")
GLOBAL_PACING_TRADE_LEDGER_PATH = Path("results/global_pacing_trade_ledger.csv")

STRATEGY_NAME = "negative_event_reversal_30d_abnormal_global_pacing"

HOLD_DAYS = 30
MAX_CONCURRENT_POSITIONS = 5
TRANSACTION_COST_BPS_PER_SIDE = 5.0

MIN_DAYS_BETWEEN_NEW_TRADES_GRID = [0, 1, 2, 3, 5, 10]
...
class Trade:
    ...
    volume_shock: float
    min_days_between_new_trades: int
...
def load_events(path: Path = EVENT_PANEL_PATH) -> pd.DataFrame:
    """Load detected negative event panel."""
    if not path.exists():
        raise FileNotFoundError(
...
def build_candidate_trades(
    ...
    Global pacing rule:
    after accepting a candidate event, do not accept any new event across the
    entire universe until min_days_between_new_trades calendar days have passed.
    """
    ...
    for _, event in events.iterrows():
        ...
        trades.append(
            Trade(
                ...
                min_days_between_new_trades=min_days_between_new_trades,
...
def summarize_run(
    ...
    """Summarize one global pacing run."""
    if accepted_trades.empty:
        return {
    ...
    return {
        ...
        "trade_win_rate": (accepted_trades["net_abnormal_return"] > 0).mean(),
        "avg_trade_net_abnormal_return": accepted_trades[
            "net_abnormal_return"
        ].mean(),
        "median_trade_net_abnormal_return": accepted_trades[
            "net_abnormal_return"
        ].median(),
...
def run_global_pacing_sensitivity() -> None:
    """Run global trade-pacing sensitivity analysis."""
    prices = load_prices()
    events = load_events()
    ...
    for min_days_between_new_trades in MIN_DAYS_BETWEEN_NEW_TRADES_GRID:
        print(
            "Running global pacing sensitivity: "
            f"{min_days_between_new_trades} calendar days between new trades..."
        )
        ...
        if not accepted_trades.empty:
            ...
            ledger_frames.append(accepted_trades)

    summary = pd.DataFrame(summary_rows)

    GLOBAL_PACING_SENSITIVITY_PATH.parent.mkdir(parents=True, exist_ok=True)
    summary.to_csv(GLOBAL_PACING_SENSITIVITY_PATH, index=False)

    if ledger_frames:
        ledger = pd.concat(ledger_frames, ignore_index=True)
        ledger.to_csv(GLOBAL_PACING_TRADE_LEDGER_PATH, index=False)

    print()
    print(f"Saved global pacing sensitivity to: {GLOBAL_PACING_SENSITIVITY_PATH}")
    print(f"Saved global pacing trade ledger to: {GLOBAL_PACING_TRADE_LEDGER_PATH}")

    print()
    print("Global pacing sensitivity summary")
    print("---------------------------------")
    print(
        summary[
            [
