...
HOLDING_PERIOD_SENSITIVITY_PATH = Path("results/holding_period_sensitivity.csv")

STRATEGY_NAME = "negative_event_reversal_abnormal"

HOLD_DAYS_GRID = [5, 10, 20, 30, 60]
...
def load_events(path: Path = EVENT_PANEL_PATH) -> pd.DataFrame:
    ...
    required = {"ticker", "date", "event_direction"}
...
def build_candidate_trades(
    ...
    if not trades:
        raise RuntimeError(f"No candidate trades generated for hold_days={hold_days}.")
...
def summarize_holding_period_run(
    trades: pd.DataFrame,
    portfolio: pd.DataFrame,
    hold_days: int,
) -> dict[str, float | int | str]:
    """Summarize one holding-period run."""
...
def run_holding_period_sensitivity() -> None:
    """Run holding-period sensitivity for negative-event reversal."""
    prices = load_prices()
    events = load_events()
    ...
    for hold_days in HOLD_DAYS_GRID:
        print(f"Running holding period sensitivity: {hold_days} trading days...")
    ...
    HOLDING_PERIOD_SENSITIVITY_PATH.parent.mkdir(parents=True, exist_ok=True)
    summary.to_csv(HOLDING_PERIOD_SENSITIVITY_PATH, index=False)

    print()
    print(f"Saved holding-period sensitivity to: {HOLDING_PERIOD_SENSITIVITY_PATH}")

    print()
    print("Holding-period sensitivity summary")
    print("----------------------------------")
    print(
        summary[
            [
...
if __name__ == "__main__":
    run_holding_period_sensitivity()